...
## 📄 Deep Dive: HTTP Response Headers & Body

Once the server processes a request, it sends back metadata via **Response Headers** followed by the actual payload inside the **Response Body**(its where the actual data lives, things like HTML,JSON,images).

### 🛡️ Common Response Headers & Security Controls

Response headers are heavily leveraged by security teams to configure defensive settings, manage state, and control how browsers handle transmitted data.

| Header Name | Technical Purpose | Cybersecurity Significance |
| :--- | :--- | :--- |
| **`Server`** | Identifies the software name and version running on the backend host (e.g., `Apache/2.4.41`). | **Information Leakage:** Attackers inspect this to find out the exact version and look up public exploits (CVEs). |
| **`Set-Cookie`** | Sends a session identifier or cookie tracking token from the server for the browser to store locally. | Must use security flags like `Secure` (HTTPS only) and `HttpOnly` to stop attackers from stealing sessions via XSS. |
| **`Content-Type`** | Dictates the exact media type of the resource (e.g., `text/html`, `image/png`) so the browser renders it right. | Hardening this prevents MIME-sniffing attacks where a browser gets tricked into running an uploaded text file as a script. |
| **`Date`** | Contains the exact date and timestamp when the HTTP response message was generated by the server. | Analysts sync system clocks with this header during incident investigations to map out chronological attack timelines. |
...
2.**Get Request to /api/users:** ![Web Application Basics Lab Screenshot](images/web-app-basics-get.png)
